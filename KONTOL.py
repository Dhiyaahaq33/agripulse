...
import random, math, json, sqlite3, threading, secrets, csv, io, time
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS sensor_readings (
        ...
        suhu REAL, kelembaban REAL, co2 REAL, ec REAL, ph REAL, vpd REAL,
    ...
    CREATE TABLE IF NOT EXISTS orders (
        ...
        user_id INTEGER,
        nomor_invoice TEXT,
        nama TEXT, email TEXT, hp TEXT, perusahaan TEXT, alamat TEXT,
        items_json TEXT, total INTEGER,
        pembayaran TEXT, status TEXT DEFAULT 'pending',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
    """)
    ...
    if not cur.fetchone():
        db.execute("INSERT INTO users(username,email,password_hash,role) VALUES(?,?,?,?)",
            ('admin', 'admin@agripulse.id', generate_password_hash('admin123'), 'admin'))
    db.commit()
    db.close()
    ...
    if session.get('user_id'):
        try:
            ...
            row = db.execute(
                ...
                "WHERE g.user_id=? AND a.resolved=0", (session['user_id'],)
            ).fetchone()
            alert_count = row['c'] if row else 0
            db.close()
        except Exception:
            pass
    return {
...
<nav>
  ...
  <div class="nav-links">
    ...
    {% if current_user %}
    <a href="/greenhouses" class="{{ 'active' if active=='greenhouses' }}">🌱 Greenhouse</a>
    <a href="/alerts"      class="{{ 'active' if active=='alerts' }}">🔔 Alerts{% if alert_count > 0 %}<span class="cart-badge">{{ alert_count }}</span>{% endif %}</a>
    {% if user_role == 'admin' %}<a href="/admin" class="{{ 'active' if active=='admin' }}">⚙️ Admin</a>{% endif %}
    <a href="/logout" style="background:rgba(231,76,60,.2);color:#ffd0cc;">👤 {{ current_user }} ⏏</a>
    {% else %}
    <a href="/login"    class="{{ 'active' if active=='login'    }}">🔑 Login</a>
    <a href="/register" class="{{ 'active' if active=='register' }}">📝 Daftar</a>
    {% endif %}
...
LOGIN_HTML = make_page("""
<div class="hero">
  <h1>🔑 Masuk ke AgriPulse</h1>
  <p>Kelola greenhouse cerdas Anda dari mana saja</p>
</div>
<div class="container" style="max-width:440px;">
  {% if error %}<div class="warn-box">{{ error }}</div>{% endif %}
  <div class="card">
    <h2>Login</h2>
    <form method="POST" action="/login">
      ...
      <button type="submit" class="btn btn-green w100">Masuk</button>
    </form>
    <p style="margin-top:14px;font-size:.87rem;text-align:center;color:#666;">
      Belum punya akun? <a href="/register" style="color:var(--gd);font-weight:600;">Daftar di sini</a>
    </p>
...
REGISTER_HTML = make_page("""
<div class="hero">
  <h1>📝 Buat Akun AgriPulse</h1>
  <p>Mulai monitoring greenhouse cerdas Anda hari ini</p>
</div>
<div class="container" style="max-width:480px;">
  ...
  <div class="card">
    ...
    <form method="POST" action="/register">
      ...
      <button type="submit" class="btn btn-green w100">Buat Akun</button>
    </form>
    <p style="margin-top:14px;font-size:.87rem;text-align:center;color:#666;">
      Sudah punya akun? <a href="/login" style="color:var(--gd);font-weight:600;">Masuk di sini</a>
    </p>
  </div>
</div>
""")
...
# ─────────────────────────────────────────────
# GREENHOUSE PAGES HTML
# ─────────────────────────────────────────────

GREENHOUSES_HTML = make_page("""
<div class="hero">
  <h1>🌱 Greenhouse Saya</h1>
  <p>Kelola & monitor semua greenhouse Anda dalam satu dashboard</p>
</div>
<div class="container">
  {% if msg %}<div class="alert-box">{{ msg }}</div>{% endif %}

  <div class="card">
    <h2>➕ Tambah Greenhouse Baru</h2>
    <form method="POST" action="/greenhouses/add"
          style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:14px;align-items:end;">
      <div class="form-group" style="margin:0;">
        <label>Nama Greenhouse</label>
        <input type="text" name="nama" required placeholder="cth: GH Selada A1">
      </div>
      <div class="form-group" style="margin:0;">
        <label>Komoditas</label>
        <select name="komoditas">
          {% for k, v in komoditas_list.items() %}
          <option value="{{ k }}">{{ v.emoji }} {{ v.nama }}</option>
          {% endfor %}
        </select>
      </div>
      <div class="form-group" style="margin:0;">
        <label>Lokasi</label>
        <input type="text" name="lokasi" placeholder="cth: Berastagi, Karo">
      </div>
      <div class="form-group" style="margin:0;">
        <label>Luas (m²)</label>
        <input type="number" name="luas" step="0.1" min="0" placeholder="200">
      </div>
      <div>
        <button type="submit" class="btn btn-green w100">➕ Tambah</button>
      </div>
    </form>
  </div>

  <div class="section-title">📋 Daftar Greenhouse ({{ greenhouses|length }} unit)</div>
  {% if not greenhouses %}
  <div class="info-box">Belum ada greenhouse. Tambahkan di atas!</div>
  {% endif %}
  <div class="produk-grid">
    {% for gh in greenhouses %}
    {% set ki = komoditas_list[gh.komoditas] if gh.komoditas in komoditas_list else komoditas_list['selada'] %}
    <div class="produk-card">
      <span class="produk-badge">{{ ki.emoji }} {{ gh.komoditas|capitalize }}</span>
      <div class="produk-emoji">🌿</div>
      <div class="produk-nama">{{ gh.nama }}</div>
      <div style="color:#888;font-size:.83rem;margin:6px 0;">
        📍 {{ gh.lokasi or 'Lokasi belum diset' }} &nbsp;|&nbsp; 📐 {{ gh.luas or 0 }} m²
      </div>
      <div style="font-size:.76rem;color:#aaa;margin-bottom:10px;word-break:break-all;">
        🔑 <code style="font-size:.73rem;">{{ gh.api_key[:24] }}...</code>
      </div>
      {% if gh.latest %}
      <div style="background:var(--gp);border-radius:8px;padding:8px 10px;font-size:.81rem;margin-bottom:12px;">
        <strong>Update terakhir:</strong> {{ gh.latest.timestamp }}<br>
        🌡️ {{ gh.latest.suhu }}°C &nbsp; 💧 {{ gh.latest.kelembaban }}% &nbsp; 🌫️ {{ gh.latest.co2 }}ppm
      </div>
      {% else %}
      <div class="info-box" style="margin-bottom:10px;font-size:.79rem;">Belum ada data sensor (simulator aktif setiap 30s)</div>
      {% endif %}
      <div style="display:flex;gap:7px;margin-top:auto;">
        <a href="/greenhouses/{{ gh.id }}" class="btn btn-green" style="flex:1;">📊 Detail</a>
        <a href="/export/csv/{{ gh.id }}" class="btn btn-dark" style="padding:10px 11px;" title="Export CSV">📥</a>
        <form method="POST" action="/greenhouses/{{ gh.id }}/delete" style="margin:0;"
              onsubmit="return confirm('Hapus {{ gh.nama }}? Data sensor ikut terhapus.')">
          <button type="submit" class="btn btn-red" style="padding:10px 11px;" title="Hapus">🗑️</button>
        </form>
      </div>
    </div>
    {% endfor %}
...
GH_DETAIL_HTML = make_page("""
<div class="hero">
  <h1>📡 {{ gh.nama }}</h1>
  <p>{{ ki.emoji }} {{ ki.nama }} &nbsp;|&nbsp; 📍 {{ gh.lokasi or '-' }} &nbsp;|&nbsp; 📐 {{ gh.luas or 0 }} m²</p>
</div>
<div class="container">
  <div style="display:flex;gap:9px;margin-bottom:16px;flex-wrap:wrap;">
    <a href="/greenhouses" class="btn btn-outline">← Kembali</a>
    <a href="/export/csv/{{ gh.id }}" class="btn btn-dark">📥 Export CSV</a>
    <form method="POST" action="/greenhouses/{{ gh.id }}/delete" style="margin:0;"
          onsubmit="return confirm('Hapus {{ gh.nama }}?')">
      <button class="btn btn-red">🗑️ Hapus</button>
    </form>
  </div>

  <div class="info-box">
    🔑 <strong>API Key ESP32:</strong> <code>{{ gh.api_key }}</code><br>
    <small>POST /api/sensor/post · field: <code>api_key</code>, suhu, kelembaban, co2, ec, ph</small>
  </div>

  {% if sensor %}
  <div class="section-title">📡 Pembacaan Terakhir — {{ sensor.timestamp }}</div>
  <div class="vpd-bar">
    <div>
      <div style="font-size:.76rem;color:#666;text-transform:uppercase;letter-spacing:1px;">VPD</div>
      <div class="vpd-val">{{ vpd }} kPa</div>
    </div>
    <div class="vpd-info"><strong>{{ vpd_label }}</strong></div>
  </div>
  <div class="sensor-grid">
    {% for key, label, icon in [('suhu','Suhu','🌡️'),('kelembaban','Kelembaban','💧'),('co2','CO₂','🌫️'),('ec','EC','⚗️'),('ph','pH','🔬')] %}
    {% set val = sensor[key] %}{% set st = statuses[key] %}
    <div class="sensor-card {{ st }}">
      <div class="sensor-label">{{ icon }} {{ label }}</div>
      ...
      <div class="sensor-range">{{ threshold[key].min }}–{{ threshold[key].max }} {{ threshold[key].unit }}</div>
    </div>
    {% endfor %}
  </div>
  <div class="card">
    <h2>⚙️ Automated Action</h2>
    <ul class="aksi-list">
      {% for a in aksi %}<li>{{ a }}</li>{% endfor %}
      {% for a in alert_msgs %}<li class="alert-item">{{ a }}</li>{% endfor %}
    </ul>
  </div>
  {% else %}
  <div class="info-box">Belum ada data sensor. Background simulator mengisi data setiap 30 detik.</div>
  {% endif %}

  {% if history %}
  <div class="card">
    <h2>📈 Historis Sensor ({{ history|length }} pembacaan terakhir)</h2>
    <div class="chart-container"><canvas id="ghChart"></canvas></div>
  </div>
  <script>
  const hd = {{ history_json | safe }};
  new Chart(document.getElementById('ghChart'), {
    type:'line',
    data:{
      labels: hd.map(h=>h.t),
      datasets:[
        {label:'Suhu (°C)',      data:hd.map(h=>h.suhu),       borderColor:'#e74c3c',backgroundColor:'rgba(231,76,60,.07)', tension:.4,pointRadius:2},
        {label:'Kelembaban (%)', data:hd.map(h=>h.kelembaban), borderColor:'#3498db',backgroundColor:'rgba(52,152,219,.07)',tension:.4,pointRadius:2},
        {label:'CO₂ ÷10',       data:hd.map(h=>h.co2/10),     borderColor:'#9b59b6',backgroundColor:'rgba(155,89,182,.07)',tension:.4,pointRadius:2},
        {label:'EC (mS/cm)',     data:hd.map(h=>h.ec),         borderColor:'#27ae60',backgroundColor:'rgba(39,174,96,.07)', tension:.4,pointRadius:2},
      ]
    },
  ...
  <div class="card">
    <h2>📋 Riwayat Pembacaan ({{ history|length }} data)</h2>
    {% if history %}
    <div style="overflow-x:auto;">
    <table class="tbl">
      <tr><th>Waktu</th><th>Suhu</th><th>RH%</th><th>CO₂</th><th>EC</th><th>pH</th><th>VPD</th></tr>
      {% for h in history|reverse %}
      <tr>
        <td style="font-size:.79rem;white-space:nowrap;">{{ h.t }}</td>
        <td>{{ h.suhu }}°C</td><td>{{ h.kelembaban }}%</td>
        <td>{{ h.co2 }}ppm</td><td>{{ h.ec }}</td><td>{{ h.ph }}</td><td>{{ h.vpd }}</td>
      </tr>
      {% endfor %}
    </table>
    </div>
    {% else %}
    <div class="info-box">Belum ada riwayat sensor.</div>
    {% endif %}
  </div>
</div>
""")
...
ALERTS_HTML = make_page("""
<div class="hero">
  <h1>🔔 Alert Log</h1>
  <p>Notifikasi parameter di luar threshold untuk semua greenhouse Anda</p>
</div>
<div class="container">
  {% if msg %}<div class="alert-box">{{ msg }}</div>{% endif %}

  <div class="stat-grid">
    <div class="stat-card rd"><div class="stat-label">Alert Belum Diselesaikan</div><div class="stat-value">{{ unresolved }}</div></div>
    <div class="stat-card g2"><div class="stat-label">Total Alert (100 terakhir)</div><div class="stat-value">{{ total_alerts }}</div></div>
  </div>

  <div class="card">
    <h2>🔴 Alert Aktif</h2>
    {% set active_alerts = alerts | selectattr('resolved', 'equalto', 0) | list %}
    {% if not active_alerts %}
    <div class="alert-box">✅ Tidak ada alert aktif — semua greenhouse dalam kondisi optimal!</div>
    {% else %}
    <div style="overflow-x:auto;">
    <table class="tbl">
      <tr><th>Waktu</th><th>Greenhouse</th><th>Pesan</th><th>Level</th><th>Aksi</th></tr>
      {% for a in active_alerts %}
      <tr>
        <td style="font-size:.79rem;white-space:nowrap;">{{ a.created_at }}</td>
        <td><strong>{{ a.gh_nama }}</strong></td>
        <td>{{ a.pesan }}</td>
        <td><span style="background:{% if a.level=='critical' %}var(--rd){% else %}var(--or){% endif %};color:#fff;padding:2px 9px;border-radius:12px;font-size:.74rem;">{{ a.level }}</span></td>
        <td>
          <form method="POST" action="/alerts/{{ a.id }}/resolve" style="margin:0;">
            <button class="btn btn-green" style="padding:5px 12px;font-size:.79rem;">✓ Resolve</button>
          </form>
        </td>
      </tr>
      {% endfor %}
  ...
  <div class="card">
    <h2>✅ Riwayat Resolved</h2>
    {% set done_alerts = alerts | selectattr('resolved', 'equalto', 1) | list %}
    {% if done_alerts %}
    <div style="overflow-x:auto;">
    <table class="tbl">
      <tr><th>Waktu</th><th>Greenhouse</th><th>Pesan</th></tr>
      {% for a in done_alerts %}
      <tr style="opacity:.6;">
        <td style="font-size:.79rem;">{{ a.created_at }}</td>
        <td>{{ a.gh_nama }}</td>
        <td>{{ a.pesan }}</td>
      </tr>
      {% endfor %}
    ...
    {% else %}
    <div class="info-box">Belum ada alert yang diselesaikan.</div>
    {% endif %}
  </div>
</div>
""")
...
ADMIN_HTML = make_page("""
<div class="hero">
  <h1>⚙️ Admin Panel</h1>
  <p>PT AgriPulse Digital Nusantara — System Administration</p>
</div>
<div class="container">
  <div class="stat-grid">
    <div class="stat-card">      <div class="stat-label">Total Users</div>    <div class="stat-value">{{ stats.users }}</div></div>
    <div class="stat-card g2">   <div class="stat-label">Total Greenhouse</div><div class="stat-value">{{ stats.greenhouses }}</div></div>
    <div class="stat-card or">   <div class="stat-label">Total Orders</div>   <div class="stat-value">{{ stats.orders }}</div></div>
    <div class="stat-card rd">   <div class="stat-label">Alert Aktif</div>    <div class="stat-value">{{ stats.alerts }}</div></div>
    <div class="stat-card bl">   <div class="stat-label">Sensor Readings</div><div class="stat-value">{{ stats.readings }}</div></div>
  </div>
  ...
  <div class="card">
    ...
    <div style="overflow-x:auto;">
    <table class="tbl">
      <tr><th>#</th><th>Username</th><th>Email</th><th>Role</th><th>Greenhouse</th><th>Joined</th></tr>
      {% for u in users %}
      <tr>
        <td>{{ u.id }}</td>
        <td><strong>{{ u.username }}</strong></td>
        <td>{{ u.email }}</td>
        <td><span style="background:{% if u.role=='admin' %}var(--gd){% else %}var(--bl){% endif %};color:#fff;padding:2px 9px;border-radius:12px;font-size:.74rem;">{{ u.role }}</span></td>
        <td>{{ u.gh_count }}</td>
        <td style="font-size:.79rem;">{{ u.created_at }}</td>
      </tr>
      {% endfor %}
    </table>
    </div>
  ...
  <div class="card">
    ...
    <div style="overflow-x:auto;">
    <table class="tbl">
      <tr><th>Invoice</th><th>Nama</th><th>Total</th><th>Pembayaran</th><th>Status</th><th>Tanggal</th></tr>
      {% for o in orders %}
      <tr>
        <td><code style="font-size:.79rem;">{{ o.nomor_invoice }}</code></td>
        <td>{{ o.nama }}</td>
        <td style="color:var(--gd);font-weight:700;">Rp {{ "{:,}".format(o.total or 0).replace(",",".") }}</td>
        <td>{{ o.pembayaran }}</td>
        <td><span style="background:var(--gl);color:#fff;padding:2px 8px;border-radius:12px;font-size:.74rem;">{{ o.status }}</span></td>
        <td style="font-size:.79rem;">{{ o.created_at }}</td>
      </tr>
      {% endfor %}
    </table>
    </div>
    {% else %}
    <div class="info-box">Belum ada order masuk.</div>
    {% endif %}
  </div>

  <div class="card">
    <h2>🌱 Semua Greenhouse</h2>
    <div style="overflow-x:auto;">
    <table class="tbl">
      <tr><th>Nama</th><th>User</th><th>Komoditas</th><th>Lokasi</th><th>Status</th><th>Created</th></tr>
      {% for g in all_greenhouses %}
      <tr>
        <td>{{ g.nama }}</td><td>{{ g.username }}</td><td>{{ g.komoditas }}</td>
        <td>{{ g.lokasi or '-' }}</td>
        <td><span style="background:{% if g.aktif %}var(--gl){% else %}#999{% endif %};color:#fff;padding:2px 8px;border-radius:12px;font-size:.74rem;">{% if g.aktif %}aktif{% else %}nonaktif{% endif %}</span></td>
        <td style="font-size:.79rem;">{{ g.created_at }}</td>
      </tr>
      {% endfor %}
    </table>
    </div>
...
def checkout():
    ...
    try:
        db = get_db()
        db.execute(
            "INSERT INTO orders(user_id,nomor_invoice,nama,email,hp,perusahaan,alamat,"
            "items_json,total,pembayaran,status) VALUES(?,?,?,?,?,?,?,?,?,?,'pending')",
            (session.get('user_id'), invoice['nomor'], invoice['nama'], invoice['email'],
             invoice['hp'], invoice['perusahaan'], invoice['alamat'],
             json.dumps(invoice['items']), invoice['total'], pembayaran)
        )
        db.commit()
        db.close()
    except Exception:
...
def api_sensor_post():
    ...
    Payload: {"api_key":"<key>","suhu":28.5,"kelembaban":72,"co2":650,"ec":1.8,"ph":6.1}
    Tanpa api_key: simulasi tanpa save ke DB.
    """
    ...
    # — Simpan ke DB jika ada api_key valid —
    api_key = data.get("api_key")
    if api_key:
        try:
            db = get_db()
            gh = db.execute("SELECT * FROM greenhouses WHERE api_key=? AND aktif=1",
                            (api_key,)).fetchone()
            if gh:
                db.execute(
                    "INSERT INTO sensor_readings(greenhouse_id,suhu,kelembaban,co2,ec,ph,vpd) "
                    "VALUES(?,?,?,?,?,?,?)",
                    (gh['id'], data.get('suhu'), data.get('kelembaban'),
                     data.get('co2'), data.get('ec'), data.get('ph'), vpd)
                )
                for msg in alerts:
                    db.execute(
                        "INSERT INTO alerts(greenhouse_id,gh_nama,pesan,level) VALUES(?,?,?,?)",
                        (gh['id'], gh['nama'], msg, 'warning')
                    )
                db.commit()
            db.close()
        except Exception:
...
def _bg_simulator():
    """Insert simulated sensor readings every 30s for all active greenhouses."""
    time.sleep(8)
    while True:
        try:
            db = get_db()
            ghs = db.execute("SELECT * FROM greenhouses WHERE aktif=1").fetchall()
            for gh in ghs:
                s = baca_sensor()
                ...
                db.execute(
                    "INSERT INTO sensor_readings(greenhouse_id,suhu,kelembaban,co2,ec,ph,vpd) "
                    "VALUES(?,?,?,?,?,?,?)",
                    (gh['id'], s['suhu'], s['kelembaban'], s['co2'], s['ec'], s['ph'], vpd)
                )
                _, alert_msgs = analisis_otomatis(s, get_threshold(gh['komoditas']))
                ...
                db.execute(
                    "DELETE FROM sensor_readings WHERE greenhouse_id=? AND id NOT IN "
                    "(SELECT id FROM sensor_readings WHERE greenhouse_id=? ORDER BY id DESC LIMIT 1000)",
                    (gh['id'], gh['id'])
                )
            db.commit()
            db.close()
...
_sim_thread = threading.Thread(target=_bg_simulator, daemon=True)
...
def login_page():
    ...
    return render_template_string(LOGIN_HTML, title="Login", active="login",
                                  error=error, cart_count=cart_count())
...
def register_page():
    ...
    return render_template_string(REGISTER_HTML, title="Daftar", active="register",
                                  error=error, cart_count=cart_count())
...
def greenhouses_page():
    db = get_db()
    ghs = db.execute(
        "SELECT * FROM greenhouses WHERE user_id=? ORDER BY created_at DESC",
        (session['user_id'],)
    ).fetchall()
    result = []
    for gh in ghs:
        latest = db.execute(
            "SELECT suhu,kelembaban,co2,ec,ph,vpd,"
            "strftime('%d/%m %H:%M',timestamp) as timestamp "
            "FROM sensor_readings WHERE greenhouse_id=? ORDER BY id DESC LIMIT 1",
            (gh['id'],)
        ).fetchone()
        d = dict(gh)
        ...
        result.append(d)
    db.close()
    return render_template_string(GREENHOUSES_HTML, title="Greenhouse Saya",
        active="greenhouses", greenhouses=result, komoditas_list=KOMODITAS,
        msg=request.args.get('msg'), cart_count=cart_count())

@app.route("/greenhouses/add", methods=["POST"])
@login_required
def greenhouse_add():
    ...
    db = get_db()
    db.execute(
        "INSERT INTO greenhouses(user_id,nama,komoditas,lokasi,luas,api_key) VALUES(?,?,?,?,?,?)",
        (session['user_id'], nama,
         request.form.get("komoditas", "selada"),
         request.form.get("lokasi", ""),
         float(request.form.get("luas", 0) or 0),
         secrets.token_hex(16))
    )
    db.commit()
    db.close()
    return redirect('/greenhouses?msg=Greenhouse+berhasil+ditambahkan')

@app.route("/greenhouses/<int:gh_id>")
@login_required
def greenhouse_detail(gh_id):
    db = get_db()
    gh = db.execute(
        "SELECT * FROM greenhouses WHERE id=? AND user_id=?",
        (gh_id, session['user_id'])
    ).fetchone()
    if not gh:
        db.close()
        return redirect('/greenhouses')
    history_rows = db.execute(
        "SELECT suhu,kelembaban,co2,ec,ph,vpd,"
        "strftime('%d/%m %H:%M',timestamp) as t "
        "FROM sensor_readings WHERE greenhouse_id=? ORDER BY id DESC LIMIT 50",
        (gh_id,)
    ).fetchall()
    db.close()
    history = [dict(r) for r in reversed(history_rows)]
    threshold = get_threshold(gh['komoditas'])
    ki = KOMODITAS.get(gh['komoditas'], KOMODITAS['selada'])
    sensor = aksi = alert_msgs = statuses = None
    vpd, vpd_label = 0, ""
    if history:
        raw = history[-1].copy()
        ...
        sensor   = raw
        statuses = {k: status_sensor(sensor[k], k, threshold) for k in threshold}
        aksi, alert_msgs = analisis_otomatis(sensor, threshold)
        vpd = hitung_vpd(sensor['suhu'], sensor['kelembaban'])
        vpd_label, _ = vpd_status(vpd)
    return render_template_string(GH_DETAIL_HTML, title=gh['nama'],
        active="greenhouses", gh=gh, ki=ki, sensor=sensor,
        threshold=threshold, statuses=statuses or {},
        aksi=aksi or [], alert_msgs=alert_msgs or [],
        vpd=vpd, vpd_label=vpd_label,
        history=history, history_json=json.dumps(history),
        cart_count=cart_count())

@app.route("/greenhouses/<int:gh_id>/delete", methods=["POST"])
@login_required
def greenhouse_delete(gh_id):
    db = get_db()
    db.execute("DELETE FROM sensor_readings WHERE greenhouse_id=?", (gh_id,))
    db.execute("DELETE FROM alerts WHERE greenhouse_id=?", (gh_id,))
    db.execute("DELETE FROM greenhouses WHERE id=? AND user_id=?", (gh_id, session['user_id']))
    db.commit()
    db.close()
    return redirect('/greenhouses')
...
def alerts_page():
    db = get_db()
    rows = db.execute(
        "SELECT a.* FROM alerts a JOIN greenhouses g ON a.greenhouse_id=g.id "
        "WHERE g.user_id=? ORDER BY a.resolved ASC, a.created_at DESC LIMIT 100",
        (session['user_id'],)
    ).fetchall()
    alerts = [dict(r) for r in rows]
    unresolved = sum(1 for a in alerts if not a['resolved'])
    db.close()
    return render_template_string(ALERTS_HTML, title="Alerts", active="alerts",
        alerts=alerts, unresolved=unresolved, total_alerts=len(alerts),
        msg=request.args.get('msg'), cart_count=cart_count())
...
def alert_resolve(alert_id):
    db = get_db()
    db.execute(
        "UPDATE alerts SET resolved=1 WHERE id=? AND greenhouse_id IN "
        "(SELECT id FROM greenhouses WHERE user_id=?)",
        (alert_id, session['user_id'])
    )
    db.commit()
    db.close()
    return redirect('/alerts?msg=Alert+berhasil+di-resolve')

# ─────────────────────────────────────────────
# CSV EXPORT
# ─────────────────────────────────────────────

@app.route("/export/csv/<int:gh_id>")
@login_required
def export_csv(gh_id):
    db = get_db()
    gh = db.execute(
        "SELECT * FROM greenhouses WHERE id=? AND user_id=?",
        (gh_id, session['user_id'])
    ...
    if not gh:
        ...
        return redirect('/greenhouses')
    rows = db.execute(
        "SELECT timestamp,suhu,kelembaban,co2,ec,ph,vpd "
        "FROM sensor_readings WHERE greenhouse_id=? ORDER BY timestamp DESC LIMIT 1000",
        (gh_id,)
    ).fetchall()
    db.close()
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['timestamp', 'suhu', 'kelembaban', 'co2', 'ec', 'ph', 'vpd'])
    for r in rows:
        writer.writerow([r['timestamp'], r['suhu'], r['kelembaban'],
                         r['co2'], r['ec'], r['ph'], r['vpd']])
    fname = f"agripulse_{gh['nama'].replace(' ','_')}_{datetime.now().strftime('%Y%m%d')}.csv"
...
def admin_panel():
    ...
    stats = {
        ...
        'greenhouses': db.execute("SELECT COUNT(*) FROM greenhouses").fetchone()[0],
        'orders':      db.execute("SELECT COUNT(*) FROM orders").fetchone()[0],
        'alerts':      db.execute("SELECT COUNT(*) FROM alerts WHERE resolved=0").fetchone()[0],
        'readings':    db.execute("SELECT COUNT(*) FROM sensor_readings").fetchone()[0],
    }
    users = db.execute(
        "SELECT u.*, (SELECT COUNT(*) FROM greenhouses WHERE user_id=u.id) as gh_count "
        "FROM users u ORDER BY created_at DESC"
    ).fetchall()
    orders  = db.execute("SELECT * FROM orders ORDER BY created_at DESC LIMIT 20").fetchall()
    all_ghs = db.execute(
        "SELECT g.*, u.username FROM greenhouses g "
        "JOIN users u ON g.user_id=u.id ORDER BY g.created_at DESC"
    ).fetchall()
    db.close()
    return render_template_string(ADMIN_HTML, title="Admin Panel", active="admin",
        stats=stats, users=[dict(u) for u in users],
        orders=[dict(o) for o in orders],
        all_greenhouses=[dict(g) for g in all_ghs],
        cart_count=cart_count())
...
if __name__ == "__main__":
    ...
    print("=" * 65)
    print()
    print("  [OK] Server berjalan di: http://localhost:5000")
    print("  [DB] Database        : agripulse.db (SQLite, auto-created)")
    print("  [BG] Background sim  : setiap 30 detik untuk semua greenhouse aktif")
    print()
    print("  Halaman Publik:")
    print("    /           -> Dashboard monitoring real-time (simulasi)")
    ...
    print()
    print("  Auth:")
    print("    /login      -> Login (default admin: admin/admin123)")
    print("    /register   -> Daftar akun baru")
    print("    /logout     -> Logout")
    print()
    print("  User (login required):")
    print("    /greenhouses        -> Kelola greenhouse Anda")
    print("    /greenhouses/<id>   -> Detail + sensor chart + tabel")
    print("    /alerts             -> Alert log + resolve")
    print("    /export/csv/<id>    -> Export CSV sensor history")
    print()
    print("  Admin (admin only):")
    print("    /admin      -> Panel admin: users, orders, analytics")
    print()
    print("  API (ESP32):")
    print("    GET  /api/sensor?komoditas=selada  -> data sensor + analisis")
    print("    POST /api/sensor/post              -> {api_key,suhu,kelembaban,co2,ec,ph}")
